...
write_df <- cbind(gsub(".", "_", row.names(logFC_by_cond), fixed = TRUE), logFC_by_cond)
colnames(write_df)[1] <- "# Gene Family" 
write.table(write_df, file = paste(out_dir_gen_data, "logFC_per_contrast.tsv", sep = "/"), sep = '\t', quote = FALSE, row.names = FALSE)
knitr::kable(logFC_by_cond[1:min(nrow(logFC_by_cond), 50), ])

# get display names
logFC_by_cond_anno <- merge(logFC_by_cond, counts_clean_anno_df, by.x = 0, by.y = 0, all.x = TRUE)

# heatmap of top significant and expressed features by contrasts
heat_FC <- plot.heatmap(logFC_by_cond_num, min(nrow(logFC_by_cond_num), 50), logFC_by_cond_anno$display_name, "Heatmap of top 50 significant, expressed features by contrasts")
...
# upset plots
upset_ids <- as.data.frame(logFC_by_cond[ , seq(2, ncol(logFC_by_cond), 2)])
...
upset(upset_ids, order.by = "freq",  nsets = ncol(upset_ids), nintersects = 40, mainbar.y.label = "number of common features between contrasts", sets.x.label = "number of signific features per contrast", keep.order = T, text.scale = 1.4, point.size = 2.6, line.size = 0.8, set_size.show = TRUE)
dev.off()
upset(upset_ids, order.by = "freq",  nsets = ncol(upset_ids), nintersects = 40, mainbar.y.label = "number of common features between contrasts", sets.x.label = "number of signific features per contrast", keep.order = T, text.scale = 1.4, point.size = 2.6, line.size = 0.8, set_size.show = TRUE)
```
