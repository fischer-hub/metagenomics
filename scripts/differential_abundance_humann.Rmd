...
### load librarys
```{r load_librarys, echo=params$show_code}
library(tidyverse)
library(broom)
library(Maaslin2)
library(foreach)
...
```{r load_files, echo=params$show_code}
...
### functions
```{r function_defs, echo=params$show_code}
...
```{r clean_names, echo=params$show_code}
...
```{r clean_data, echo=params$show_code}
...
```{r id_mapping, echo=params$show_code}
...
### plots on count data
```{r count_plots, echo=params$show_code, message=FALSE}
...
```{r general_model, echo=params$show_code,  message=FALSE}
...
### run comparisons
```{r contrast_models, echo=params$show_code}
...
worker_array <- foreach(i = 1:nrow(comparisons_df), .errorhandling='pass', .verbose=TRUE, .packages = c("Maaslin2", "EnhancedVolcano", "pheatmap", "tidyverse", "doParallel")) %dopar% {
  ...
  bin <- write.csv(top_100_df, file = paste(out_dir_con_data, paste(contrast, "top_100_features.csv", sep = "_"), sep = "/"))
  ...
  try({
    ...
    bin <- ggsave(filename = paste(contrast, "pca.png"), plot = pca_con, device = png, path = out_dir_con_plots, height = params$plot_height, width = params$plot_width)
    pca_con_log <- plot.pca(log2(counts_clean_df[, 1:(length(colnames(counts_clean_df)) - 2)]), metadata_df, cond, cond)
    bin <- ggsave(filename = paste(contrast, "pca_log.png"), plot = pca_con_log, device = png, path = out_dir_con_plots, height = params$plot_height, width = params$plot_width)
  }, silent=TRUE)
  ...
  try({
    ...
    bin <- ggsave(filename = paste(contrast, "heatmap_top_50_count_by_FC.png", sep = "_"), plot = heat_con, device = png, path = out_dir_con_plots, height = params$plot_height, width = params$plot_width)
    ...
    bin <- ggsave(filename = paste(contrast, "heatmap_top_50_log_count_by_FC.png", sep = "_"), plot = heat_con_log, device = png, path = out_dir_con_plots, height = params$plot_height, width = params$plot_width)
  }, silent=TRUE)
  ...
  try({
    ...
    bin <- ggsave(filename = paste(contrast, "sample_to_sample_dist_count.png", sep = "_"), plot = sample_dist_con, device = png, path = out_dir_con_plots, height = params$plot_height, width = params$plot_width)
    ...
    bin <- ggsave(filename = paste(contrast, "sample_to_sample_dist_log_count.png", sep = "_"), plot = sample_dist_log, device = png, path = out_dir_con_plots, height = params$plot_height, width = params$plot_width)
  }, silent=TRUE)
  ...
  try({
    ...
    bin <- ggsave(filename = paste(contrast, "volcano_plot.png", sep = "_"), plot = volcano_plot_gen, device = png, path = out_dir_con_plots, width = 15, height = 10)
  }, silent=TRUE)
  
  try({
    # ma-plot for current contrast
    ma_plot_con <- plot.ma(results_cond_df, contrast)
    bin <- ggsave(filename = paste(contrast, "ma_plot.png", sep = "_"), plot = ma_plot_con, device = png, path = out_dir_gen_plots, height = params$plot_height, width = params$plot_width)
  }, silent=TRUE)
