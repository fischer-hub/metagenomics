...
for(i in 1:length(rownames(comparisons_df))){
  ...
  results_df <- results_df[results_df$metadata %in% cond, ]
  ...
  top_100_df <- res_sig_ordered_df[1:min(length(rownames(res_sig_ordered_df)), 100), ]
  ...
  heat_con <- plot.heatmap(top_100_num_df, min(length(rownames(top_100_num_df)), 50), top_100_anno_df$display_name, paste("Heatmap of top 50 significant feature counts by logFC colored by", cond, sep = " "), group = groups)
  ...
  heat_con_log <- plot.heatmap(log2(counts_clean_num_df), 50, counts_clean_anno_df$display_name, paste("Heatmap of top 50 significant feature counts by logFC colored by", cond, "(log)", sep = " "))
