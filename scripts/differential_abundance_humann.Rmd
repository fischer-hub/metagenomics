...
for(i in 1:length(rownames(comparisons_df))){
  ...
  if(!(contrast %in% colnames(logFC_by_cond))){
    temp <- as.data.frame(results_df[results_df$value %in% alt  , "logFC"])
    temp$feature <- results_df[results_df$value %in% alt  , "feature"]
    temp$qval <- results_df[results_df$value %in% alt  , "qval"]
...
ggsave(filename = paste(contrast, "heatmap_top_50_FC_by_contrast.png", sep = "_"), plot = heat_FC, device = png, path = out_dir_gen_plots, height = params$plot_height, width = params$plot_width)
...
### parallel stuff
```{r echo=params$params$show_code}
