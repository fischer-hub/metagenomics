...
plot.pca <- function(df, meta, group){
  ...
  ggplot(data = dtp) + 
       ...
       labs(color = group) +
       ggtitle(paste("PCA", title_txt, sep = ": ")) +
       theme(plot.title = element_text(hjust = 0.5)) +
       coord_fixed()
}

plot.heatmap <- function(df, n, anno, cluster_rows = TRUE, cluster_cols = TRUE){
  pheatmap(df[1:n, ], scale = "row", annotation_row = anno, main = title_txt,
