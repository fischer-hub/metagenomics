...
plot.heatmap <- function(df, n, anno, title_txt, cluster_rows = TRUE, cluster_cols = TRUE, group = NA){
  pheatmap(df[1:n, ], scale = "row", annotation_row = group, main = title_txt,
        labels_row = anno[1:n],
...
plot.dist <- function(df, title_txt, group = NA){
  pheatmap(dist(df), cluster_cols = FALSE, cluster_rows = FALSE,
...
ggsave(filename = "pca_general.png", plot = pca_gen, device = png, path = out_dir_gen_plots)
pca_gen_log <- plot.pca(log2(counts_clean_df[, 1:(length(colnames(counts_clean_df)) - 2)]), metadata_df, "sample", "sample")
ggsave(filename = "pca__log_general.png", plot = pca_gen_log, device = png, path = out_dir_gen_plots)

# heatmap of top 50
heat_gen <- plot.heatmap(counts_clean_num_df, 50, counts_clean_anno_df$display_name, "Heatmap top 50 counts")
ggsave(filename = "heatmap_top_50_count_gen.png", plot = heat_gen, device = png, path = out_dir_gen_plots)
heat_gen_log <- plot.heatmap(log2(counts_clean_num_df), 50, counts_clean_anno_df$display_name, "Heatmap top 50 counts")
ggsave(filename = "heatmap_top_50_count_log_gen.png", plot = heat_gen_log, device = png, path = out_dir_gen_plots)

# sample to sample distance
sample_dist <- plot.dist(t(counts_clean_num_df), "Sample to sample count distance (euclidean)")
ggsave(filename = "sample_to_sample_dist_count.png", plot = sample_dist, device = png, path = out_dir_gen_plots)
sample_dist_log <- plot.dist(t(log2(counts_clean_num_df)), "Sample to sample log count distance (euclidean)")
ggsave(filename = "sample_to_sample_dist_log_count.png", plot = sample_dist_log, device = png, path = out_dir_gen_plots)
```
...
ggsave(filename = "ma_plot_general.png", plot = ma_plot_gen, device = png, path = out_dir_gen_plots)

volcano_plot_gen <- plot.volcano(results_df, "Total differential abundant features", results_df$display_name, "all groups")
ggsave(filename = "volcano_plot_general.png", plot = volcano_plot_gen, device = png, path = out_dir_gen_plots)
```
