...
params:
  counts: "/Users/user/Downloads/sim_genefamilies_cpm_combined.tsv"
  metadata: "/Users/user/Downloads/SraRunTable.txt"
  show_code: TRUE
  comparisons: "cond,seed"
  cpus: 4
  count_threshold: 2
  alpha: 0.05
  fc_threshold: 1
...
# run with: cript -e "rmarkdown::render('differential_abundance_humann.Rmd',params=list(counts = '', metadata = ''))"
...
pacman::p_load(DESeq2, tidyverse, broom, Maaslin2, foreach, doParallel, httr, jsonlite, xml2, EnhancedVolcano, UniProt.ws, ggplot2, pheatmap)
```
...
plot.volcano <-function(df, title_txt, label_col, subtitle_txt){
  EnhancedVolcano(df,
                  ...
                  title = title_txt, 
                  subtitle = paste("Volcano plot", subtitle_txt, sep = ": "),
                  pCutoff = params$alpha,
                  FCcutoff = params$fc_threshold,
...
plot.pca <- function(df, meta, group){
  ...
  autoplot(pca, data = meta, colour = group) +
    ggtitle(paste("PCA colorized by variable", group, sep = ": ")) +
    theme(plot.title = element_text(hjust = 0.5))
}

### T0D0: split if input list is bigger than 10k entries, call recursivly until < 10k
#im too scraed to test this
...
uniprot.map <- function(ids1, ids2, from_str, to_str, mapping_df){
  ...
  if(length(ids1) > 1000){
    uniprot.map(ids1[1:10000],  ids1[10000:length(ids1)], from_str, to_str, mapping_df)
  }
  if(length(ids1) <= 10000){
    response <- POST(url = "https://www.uniprot.org/uploadlists/",
                    body = list(from = from_str,
                                to = to_str,
                                format = 'tab',
                                query = paste(ids1, collapse = ' ')))
  
    mapping <- rbind(mapping, httr::content(response, type = 'text/tab-separated-values', encoding = "UTF-8"))
    uniprot.map(ids2[1:min(length(ids2), 10000)], ids2[min(length(ids2), 10000):length(ids2)], from_str, to_str, mapping_df)
  }
}
```
...
counts_clean_df <- na.omit(counts_clean_df)

# add pseudo count to prevent MaAslin2 model fit from crashing
counts_clean_df <- counts_clean_df+1
...
hist(rowSums(counts_clean_df[-1, ]), br = 100, main = "Cleaned gene count distribution over all samples", xlab = "Genes counts")
hist(log2(rowSums(counts_clean_df[-1, ])), br = 100, main = "Log2 transformed, cleaned gene count distribution over all samples", xlab = "Log2 transformed genes counts")
```

### set feature display names
```{r echo=params$show_code}
# map uniref IDs to genename
  ...
  feature_names <- as.data.frame(uniprot.map(feature_ids, c(), from, "GENENAME"), row.names = "From", data.frame())
...
pca_gen <- plot.pca(counts_clean_df, metadata_df, "sample")
pca_gen_log <- plot.pca(log2(counts_clean_df), metadata_df, "sample")

#test <- as.data.frame(rownames(counts_clean_df))
#plot.pca(t(counts_clean_df), test, "rownames(counts_clean_df)")
# heatmap
pheatmap(dist(counts_clean_df[1:10, ]), cluster_cols = FALSE, cluster_rows = TRUE,
          scale = "row", border_color = NA,
          labels_row = as.character(rownames(counts_clean_df)[1:10]),
          labels_col = as.character(colnames(counts_clean_df)),
          height = 15, width = 15,
...
worker_array <- foreach(i = 1:length(comparisons_ls), .packages = c("Maaslin2", "EnhancedVolcano", "httr", "dplyr", "ggplot2", "pheatmap")) %dopar% {
  
  out_dir <- ifelse(is.na(strsplit(comparisons_ls[i], ":")[[1]][2]), 
                    strsplit(comparisons_ls[i], ":")[[1]][1], 
                    paste(strsplit(comparisons_ls[i], ":")[[1]][1], strsplit(comparisons_ls[i], ":")[[1]][2], sep = "+"))
